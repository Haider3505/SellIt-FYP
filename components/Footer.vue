<template>
  <!--FOOTER-->
  <div>
    <footer>
      <div class="navLeftFooter">
        <a href="#" id="navBackToTop">
          <div class="navFooterBackToTop">
            <span class="navFooterBackToTopText">Back to top</span>
          </div>
        </a>
        <div class="container-fluid">
          <div class="container">
            <div class="row">
              <div class="col-md-3 col-sm-6">
                <div class="navFooterCol">
                  <div class="navFooterColHead">Get to Know Us</div>
                  <ul>
                    <li>
                      <a href="#">Careers</a>
                    </li>
                    <li>
                      <a href="#">Blog</a>
                    </li>
                    <li>
                      <a href="#">About Sell-It</a>
                    </li>
                    <li>
                      <a href="#">Investor Relations</a>
                    </li>
                    <li>
                      <a href="#">Sell-It Devices</a>
                    </li>
                    <li>
                      <a href="#">Sell-It Tours</a>
                    </li>
                  </ul>
                </div>
              </div>
              <div class="col-md-3 col-sm-6">
                <div class="navFooterCol">
                  <div class="navFooterColHead">Make Money with Us</div>
                  <ul>
                    <li>
                      <a href="#">Sell on Sell-It</a>
                    </li>
                    <li>
                      <a href="#">Blog</a>
                    </li>
                    <li>
                      <a href="#">About Sell-It</a>
                    </li>
                    <li>
                      <a href="#">Investor Relations</a>
                    </li>
                    <li>
                      <a href="#">Sell-It Devices</a>
                    </li>
                    <li>
                      <a href="#">Sell-It Tours</a>
                    </li>
                  </ul>
                </div>
              </div>
              <div class="col-md-3 col-sm-6">
                <div class="navFooterCol">
                  <div class="navFooterColHead">Sell-It Payment Products</div>
                  <ul>
                    <li>
                      <a href="#">Sell on Sell-It</a>
                    </li>
                    <li>
                      <a href="#">Blog</a>
                    </li>
                    <li>
                      <a href="#">About Sell-It</a>
                    </li>
                    <li>
                      <a href="#">Investor Relations</a>
                    </li>
                    <li>
                      <a href="#">Sell-It Devices</a>
                    </li>
                    <li>
                      <a href="#">Sell-It Tours</a>
                    </li>
                  </ul>
                </div>
              </div>
              <div class="col-md-3 col-sm-6">
                <div class="navFooterCol">
                  <div class="navFooterColHead">Let Us Help You</div>
                  <ul>
                    <li>
                      <a href="#">Sell on Sell-It</a>
                    </li>
                    <li>
                      <a href="#">Blog</a>
                    </li>
                    <li>
                      <a href="#">About Sell-It</a>
                    </li>
                    <li>
                      <a href="#">Investor Relations</a>
                    </li>
                    <li>
                      <a href="#">Sell-It Devices</a>
                    </li>
                    <li>
                      <a href="#">Sell-It Tours</a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div class="nav-footer-line"></div>
          <!-- <div class="container">
            <div class="navFooterPadItem row">
              <div class="col-md-4 col-sm-12 text-right">
                <div class="navFooterLogoLine">
                  <a href="#">
                    <div class="nav-logo-base nav-sprite"></div>
                  </a>
                </div>
              </div>
              <div class="col-md-8 col-sm-12">
                <span class="icp-container-desktop">
                  <div class="navFooterLine">
                    <a href="#" class="icp-button">
                      <div
                        class="
                          icp-nav-globe-img-2
                          icp-button-globe-2
                          icp-nav-globe-curr-fix
                        "
                      ></div>
                      <span class="icp-color-base">English</span>
                      <span class="nav-arrow icp-up-down-arrow"></span>
                    </a>
                    <a href="#" class="icp-button">
                      <span class="icp-currency-symbol">Rs:</span>
                      <span class="icp-color-base">Pkr - PK Rupee</span>
                    </a>
                    <a href="#" class="icp-button">
                      <span class="icp-currency-symbol">Pkr</span>
                      <span class="icp-color-base">Pakistan</span>
                    </a>
                  </div>
                </span>
              </div>
            </div>
          </div> -->
        </div>
        <div class="navFooterDescLine">
          <div class="container-fluid">
            <div class="container">
              <table>
                <tbody>
                  <tr>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Music
                        <br />
                        <span class="navFooterDescText">
                          Stream millions
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td class="navFooterDescSpacer" style="width: 4%"></td>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer third"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem third">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer forth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem forth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer fivth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem fivth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer sixth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem sixth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer svnth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem svnth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
                  </tr>
                  <tr>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Music
                        <br />
                        <span class="navFooterDescText">
                          Stream millions
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td class="navFooterDescSpacer" style="width: 4%"></td>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer third"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem third">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer forth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem forth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer fivth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem fivth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer sixth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem sixth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer svnth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem svnth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
                  </tr>
                  <tr>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Music
                        <br />
                        <span class="navFooterDescText">
                          Stream millions
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td class="navFooterDescSpacer" style="width: 4%"></td>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer third"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem third">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer forth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem forth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer fivth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem fivth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer sixth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem sixth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer svnth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem svnth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
                  </tr>
                  <tr>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Music
                        <br />
                        <span class="navFooterDescText">
                          Stream millions
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td class="navFooterDescSpacer" style="width: 4%"></td>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer third"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem third">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer forth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem forth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer fivth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem fivth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer sixth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem sixth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer svnth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem svnth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
                  </tr>
                  <tr>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Music
                        <br />
                        <span class="navFooterDescText">
                          Stream millions
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td class="navFooterDescSpacer" style="width: 4%"></td>
                    <td class="navFooterDescItem">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer third"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem third">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer forth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem forth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer fivth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem fivth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer sixth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem sixth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                    <td
                      class="navFooterDescSpacer svnth"
                      style="width: 4%"
                    ></td>
                    <td class="navFooterDescItem svnth">
                      <a href="#" class="nav_a">
                        Sell-It Advertising
                        <br />
                        <span class="navFooterDescText">
                          Find, attract, and
                          <br />of songs
                        </span>
                      </a>
                    </td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>
        <div class="navFooterCopyright">
          <ul>
            <li>
              <a href="#" class="nav_a">Conditions of Use</a>
            </li>
            <li>
              <a href class="nav_a">Privacy Notice</a>
            </li>
            <li>
              <a href class="nav_a">Internet-Based Ads</a>
            </li>
            <li class="nav_last">
              © 1996-2019, Sell-It.com, Inc. or its affiliates
            </li>
          </ul>
        </div>
      </div>
    </footer>
    <!--/FOOTER-->
    <!-- <Nuxt /> -->
  </div>
</template>
